...
bool bellman_ford_algo(vector<EDGE>& edges_list, const UI N, const UI M, UI& start_vert, vector<int>& s_paths, const bool& directed) {
...
//    vector<EDGE> edges_list = G.get_edges_list();
    ...
    if (x == -1) {
//        cout << "No negative cycle from vertex " << start_vert << endl;
        s_paths = d;
        return 1;
    }
    else {
